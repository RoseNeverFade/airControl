...
public class DeltaLatFrag extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.fragment_delta_lat, container, false);

        droneData = DroneData.getInstance();
        LineChart chart = v.findViewById(R.id.chart_delta_lat);

        List<Entry> entries = new ArrayList<Entry>();
        // turn your data into Entry objects
        ArrayList<Double> latData = droneData.getDeltaLat();
        int i = 0;
        for (double d : latData) {
            entries.add(new Entry(i, (float) (d)));
            i++;
        }
        LineDataSet dataSet = new LineDataSet(entries, "纬度差"); // add entries to dataset
...
//        chart.getAxisLeft().setValueFormatter(new ValueOffsetFormatter(droneData.latAnchor));
        ...
        chart.getDescription().setText("纬度变化量曲线");
//        dataSet.setColor(...);
